...
{ stdenv, bazel, coreutils, enableNixHacks ? true }:
...
in stdenv.mkDerivation (fBuildAttrs // {
  ...
  deps = stdenv.mkDerivation (fFetchAttrs // {
    ...
    preConfigure = stdenv.lib.optionalString stdenv.hostPlatform.isDarwin ''
      # Explicitly configure gcov since we don't have it on Darwin, so autodetection fails
      export GCOV=${coreutils}/bin/false
    '' + fFetchAttrs.preConfigure or "";
    ...
    installPhase = fFetchAttrs.installPhase or ''
      ...
      find $bazelOut/external -type l | while read symlink; do
        ln -nsf $(readlink "$symlink" | sed "s,$NIX_BUILD_TOP,NIX_BUILD_TOP,") "$symlink"
      done
